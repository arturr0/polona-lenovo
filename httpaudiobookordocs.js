...
async function searchOnGoogle(query) {
  try {
    ...
    $('.tF2Cxc').each((index, element) => {
      ...
      // Exclude links with unsupported file types
      const excludedExtensions = ['.pdf', '.csv', '.xlsx', '.docx', '.pptx'];
      if (title && link && !excludedExtensions.some(ext => link.endsWith(ext)) && link.includes('gov.pl')) {
...
async function fetchPageContent(url, title) {
  try {
    ...
    const fileExtensions = ['.pdf', '.csv', '.xlsx', '.docx', '.pptx'];
