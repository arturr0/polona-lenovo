...
function checkInternetConnection() {
  return new Promise((resolveCallback) => {
    resolve('google.com', (err) => {
      resolveCallback(!err);  // Resolve based on whether the DNS resolve was successful
    });
  });
}
...
async function main() {
  ...
  try {
    ...
    for (const hit of polonaData.hits) {
      ...
      try {
        const googleResults = await searchOnGoogle(title);  // Use the retryable version of searchOnGoogle
        ...
        if (error.response && error.response.status === 429) {
          ...
          // Exit the program immediately if a 429 error occurs
          return;
        } else {
