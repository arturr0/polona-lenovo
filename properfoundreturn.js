...
async function fetchPageContent(url, title) {
  try {
    ...
    const hasExactTitle = pageText.includes(title); // Exact title match
...
async function downloadFile(url, title) {
  try {
    const fileName = path.basename(url.split('?')[0]); // Remove query parameters from the URL
    const filePath = path.resolve('./downloads', fileName); // Save files in a "downloads" directory
    fs.mkdirSync('./downloads', { recursive: true }); // Ensure the folder exists
    ...
    return new Promise((resolve, reject) => {
      writer.on('finish', async () => {
        ...
        try {
          const found = await checkFileContent(filePath, title);  // Await checkFileContent here
          if (found) {
            resolve(true);
          } else {
            resolve(false);  // Resolve as false if title not found
          }
        } catch (err) {
          reject(err);
        }
    ...
    console.error('Error downloading file:', error.message);
    throw error;  // Throw error here to propagate it if download fails
  }
}
...
async function checkFileContent(filePath, title) {
  try {
    ...
    return false;  // Return false on failure to check content
  }
}
