...
const advancedSearch = async (searchQuery) => {  // Dodaj parametr searchQuery
  const url = `https://polona.pl/api/search-service/search/advanced?page=0&pageSize=24&sort=OLDEST`;

  const body = {
    fieldQueries: {
      publishPlace: [{ isExact: false, query: searchQuery }],  // Użyj searchQuery
      creator: [{ isExact: false, query: searchQuery }],
      keywords: [{ isExact: false, query: 'historia' }]
    },
    filters: {
      keywordFilters: {
        copyright: ['false']
...
// Wywołanie funkcji
simpleSearch('historia');
//advancedSearch('Królewiec');  // Teraz przekazujesz searchQuery
