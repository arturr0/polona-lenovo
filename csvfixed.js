...
async function checkFileContent(filePath, title) {
  try {
    ...
    if (fileExt === '.pdf') {
      ...
      if (content.includes(title) && content.includes('audiobook')) {
        console.log(`Found audiobook and title in PDF file: ${filePath}`);
      }
      ...
      if (found) {
        console.log(`Found audiobook and title in XLS/XLSX file: ${filePath}`);
      }
    } 
    else if (fileExt === '.csv') {
      ...
      // Log parsed CSV data for better inspection
      //console.log('Parsed CSV Records:', records);
    
      // Check for specific data in CSV rows
      // const foundInCsv = records.some(row => {
      //   const rowString = JSON.stringify(row);
      //   //if(rowString.includes('Nazwa projektu')) console.log('found on csv');
      //   return rowString.includes(title) && rowString.includes('audiobook');
      // });
      const foundInCsv = records.some(row => {
        const rowString = JSON.stringify(row);
        // Use a regex to account for word boundaries and search for the title in a more robust way
        const titleRegex = new RegExp(title, 'i'); // 'i' for case-insensitive matching
        return titleRegex.test(rowString) && rowString.includes('audiobook');
      });
      
    
      if (foundInCsv) {
        console.log("Found title and audiobook in CSV");
      } else {
